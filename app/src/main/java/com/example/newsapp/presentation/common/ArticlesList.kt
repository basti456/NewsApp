...
fun ArticlesList(
    ...
    LazyColumn(
        ...
        items(count = articles.size) {
            ...
            ArticleCard(onClick = { onClick(article) }, article = article)
...
fun ArticlesList(
    ...
    if (handlePagingResult) {
        LazyColumn(
            ...
            val filteredArticles = articles.itemSnapshotList.filter {
                it?.content?.contains(selectedCategory)!! || selectedCategory == "All"
            }
            items(count = articles.itemCount) { i ->
                articles[i]?.let {
                    ArticleCard(onClick = { onClick(it) }, article = it)
                }
            }
        }
    }
